...
export default function MyOrderItems({ component }) {
      ...
      {orderStatusHistoryWithOrderId.length > 0 &&
        orderStatusHistoryWithOrderId.map((value, index) => {
          if (value.status_change_to === "PENDING")
            status = "Đơn hàng của bạn đang chờ xác nhận";
            ...
            status = "Đơn hàng của bạn đang được xử lý";
          else if (value.status_change_to === "SHIPPING")
            status =
              "Đơn hàng của bạn đang được bàn giao cho đơn vị vận chuyển";
          else if (value.status_change_to === "COMPLETED")
            status = "Đơn hàng của bạn đã được giao thành công";
          ...
          orderItemWithOrderId.map((value, index) => {
            return (
              <tr key={index}>
                ...
                {user.role === "admin" && <th>{value.stock_quantity}</th>}
              </tr>
            );
          })}
      </table>
